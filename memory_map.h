...
namespace Hw {
	...
	namespace Mm {
		...
		Memory_region const core_NW(); // Normal World
...
struct Hw::Nw_space : Hw::Memory_region_array
{
	using Hw::Memory_region_array::Memory_region_array;
	...
	{
		//Genode::log("~~~~~~~~~~~~~~~~~~~~~~~~~~NW MEM~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
		addr_t virt_base = Mm::core_NW().base;
		auto lambda = [&] (Memory_region const & r) {
			f(Mapping { r.base, virt_base, r.size, PAGE_FLAGS_KERN_DATA });
			virt_base += r.size + get_page_size();
		};
		for_each(lambda);
	}
	...
	addr_t virt_addr(addr_t phys_addr) const
	{
		...
		for_each_mapping([&] (Mapping const & m)
		{
			//if (phys_addr >= 0x80000000 && phys_addr < 0x90000000)
			//	Genode::log("Physical address: ", phys_addr);
			if (phys_addr >= m.phys() && phys_addr < (m.phys()+m.size()))
				ret = m.virt() + (phys_addr-m.phys());
		});
		return ret;
