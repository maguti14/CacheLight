  ...
  core_elf(core_elf_addr)
{
	...
	/* map Normal World Memory region in secure world */
	//core_pd->map_insert(Mapping(Trustzone::NONSECURE_RAM_BASE, Hw::Mm::core_NW().base,
	//                            Trustzone::NONSECURE_RAM_SIZE/get_page_size(), Hw::PAGE_FLAGS_KERN_DATA));
	board.core_nw.for_each_mapping([&] (Mapping const & m) {
		core_pd->map_insert(m); });
	...
	Boot_info & bootinfo =
		*construct_at<Boot_info>(bi_base, (addr_t)&core_pd->table,
		                         ...
		                         board.core_mmio, board.core_nw,
		                         board.acpi_rsdp, board.framebuffer);
